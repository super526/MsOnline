...
def send_register_email(email, send_type="register"):
    ...
    if send_type == "register":
        ...
        email_body = "请点击下面的链接重置你的密码:http://127.0.0.1:8000/reset/{0}".format(verify_code)
        send_status = send_mail(email_title, email_body, EMAIL_FROM, [email])
        if send_status:
            pass
    elif send_type == "update":
        email_title = "慕学在线网修改邮箱链接"
        email_body = "你的邮箱验证码为:{0}".format(verify_code)
        send_status = send_mail(email_title, email_body, EMAIL_FROM, [email])
        if send_status:
            pass
